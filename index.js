...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=:+62 851-5973-0633\n'
            + 'END:VCARD'
prefix = '.'
blocked = []
...
async function starts() {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				try {
					...
					ppimg = 'https://th.bing.com/th/id/OIP.5V6lnOi88XCroYUzJEqGDAHaF7?pid=ImgDet&rs=1'
				}
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["085159730633@s.whatsapp.net"]
               ...
               break
				case 'demote':
					...
					if (mentioned.length > 1) {
						teks = 'Perintah di terima, menghapus jabatan admin :\n'
						...
						mentions(`Perintah di terima, menghapus jabatan admin : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
                  case 'promote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Perintah di terima, menambah jabatan admin :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Perintah di terima, menambah jabatan admin : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				  case 'wa.me':
	...
	case 'hidetag':
		...
		case 'infonomor':
            ...
            case 'covid':
				...
				case 'testime':
					setTimeout( () => {
					client.sendMessage(from, 'Waktu habis', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 Mention All 〙✪══\n╠➥'+teks+'╚═〘 SNOWY BOT 〙', members_id, true)
					break
                case 'tagall2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Mention All 〙✪══\n╠➥'+teks+'╚═〘 SNOWY BOT 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
           ...
           client.sendMessage(from, options, text, { quoted: mek } )
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						client.groupRemove(from, mentioned)
					client.sendMessage(mentioned, 'yahaha di kick😂', text)
					}
					break
				case 'exe':
	              client.updatePresence(from, Presence.composing) 
